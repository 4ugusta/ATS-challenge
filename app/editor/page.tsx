  ...
  const handleAgenticAI = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setAiSuggestions([{ summary: "Error fetching suggestions", modifiedCV: finalText }]);
      } else {
        setAiSuggestions(data.suggestions || []);
      }
    } catch (error) {
      console.error(error);
      setAiSuggestions([{ summary: "Unable to fetch suggestions", modifiedCV: finalText }]);
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button onClick={handleAnonymize} disabled={anonymizeLoading} variant="outline">
            {anonymizeLoading ? "Anonymizing..." : "Anonymize"}
          </Button>
          <Button onClick={handleReformat} disabled={!anonymizedText || reformatLoading}>
            {reformatLoading ? "Formatting..." : "Format CV"}
          </Button>
          {formattedCV && (
            <Button onClick={loadFormattedCV}>
              Load Formatted CV
            </Button>
          )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            />
            <Button onClick={handleAgenticAI} disabled={!finalText || agentLoading}>
                ...
                {aiSuggestions.map((sugg, idx) => (
                  <Card key={idx} className="border">
                    <CardContent className="p-4">
                      ...
                      <div className="mt-2">
                        <CVDiffViewer oldText={finalText} newText={sugg.modifiedCV} />
                      </div>
                      <Button
                        variant="secondary"
                        size="sm"
